...
Maestrano['default'].configure do |config|
  ...
  #
  config.api.host = Rails.env.development? ? 'http://localhost:3000' : 'https://maestrano.com'
...
Maestrano['nab-production'].configure do |config|
  ...
  if Rails.env.development?
    ...
    config.app.host = 'https://connector-salesforce.herokuapp.com'
  end
  ...
  config.sso.init_path = '/maestrano/auth/saml/init/nab-production'
